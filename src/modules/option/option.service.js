...
class OptionService {
    ...
    async create(optionDto) {
        ...
        this.alreadyExistByCategoryAndKey(optionDto.key, category._id);
    ...
    async findById(id) {
        return await this.#model.findById(id, {__v: 0}).populate(
            [{path: "category" , select: {name: 1, slug: 1}}]
        );
    }
    async find() {
        const options = await this.#model.find({}, {__v: 0}, {sort: {id: -1}}).populate(
            [{path: "category" , select: {name: 1, slug: 1}}]
        );
        return options
    }
